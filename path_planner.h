...
const int occupancyGridOccupied = 100;
const OutOfBoundsException outOfBounds;
const cantReadFileException cantReadFile;
const int maxInt = numeric_limits<int>::max();
const double infinity = numeric_limits<double>::infinity();
const GpsInfo globalGpsGoalLocation(8.0e-6, 0.0); // Our gps goal location. For now it's preprogrammed
const bool useCostFunction = true;
const int obstacleExpansionDistance = 2; // how many cells to expand obstacles
const bool useBlinders = false;
const double blindersAngleD = 20;
const int blindersDisplacement = 10;
...
// Abstract base class for goals that A* can plan towards
class Goal
{
public:
...
class LineGoal : public Goal
{
public:
    LineGoal(double x1, double y1, double x2, double y2)
